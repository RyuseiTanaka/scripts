...
#my module
from scripts.loss_process import FOW, fow_loss
from scripts.area_process import get_cfg2reference_area
import sys
...
class FastRCNNFocaltLossBoundaryVarOutputLayers(FastRCNNOutputLayers):
    ...
    def set_fow_param(self, cfg):
        self.fow_flag = cfg.FOW.WEIGHT_ON
        if self.fow_flag:
            self.reference_area, self.cls_gamma = get_cfg2reference_area(cfg)
            print("referece_area:",self.reference_area)
            print("cls_gamma:",self.cls_gamma)
    ...
    def losses(self, predictions, proposals, branch):
        ...
        if self.fow_flag:
            w = FOW(gt_boxes, gt_classes, self.reference_area, self.cls_gamma)
        else:
            w = None
        if branch == "supervised":
            losses = {
                "loss_cls": self.comput_focal_loss(scores, gt_classes, fow=w),
                "loss_box_reg": self.box_reg_loss(
                    proposal_boxes,
                    gt_boxes,
                    proposal_deltas,
    ...
    def comput_focal_loss(self, pred_class_logits, gt_classes, fow=None):
        if gt_classes.numel() == 0:
            ...
            if fow is not None:
                total_loss = fow_loss(total_loss, fow)
            else:
                total_loss.sum()
...
# focal loss (ICLR 21 unbiased teacher)
